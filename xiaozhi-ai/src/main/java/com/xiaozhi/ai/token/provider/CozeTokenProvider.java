...
public class CozeTokenProvider implements TokenProvider {
    ...
    public TokenCache fetchToken(ConfigBO config) {
        try {
            ...
            throw new OperationFailedException("刷新Coze Token失败: " + e.getMessage(), e);
        }
    }
    ...
    private String generateJwt(ConfigBO config) throws Exception {
        ...
        if (headerJson == null || claimsJson == null) {
            throw new OperationFailedException("生成Coze JWT失败");
        }
    ...
    private TokenCache requestAccessToken(String jwt) throws Exception {
        ...
        if (response.getStatusCode() != HttpStatus.OK || response.getBody() == null) {
            throw new OperationFailedException("Coze API返回错误，HTTP状态码: " + response.getStatusCode()
                    + ", 响应: " + response.getBody());
        }
        ...
        if (accessToken == null || accessToken.isEmpty()) {
            throw new OperationFailedException("响应中未找到access_token字段");
        }
