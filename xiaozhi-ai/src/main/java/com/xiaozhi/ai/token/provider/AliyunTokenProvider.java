    ...
    public TokenCache fetchToken(ConfigBO config) {
        try {
            ...
            if (response.getHttpStatus() != 200) {
                throw new OperationFailedException("阿里云API返回错误，HTTP状态码: " + response.getHttpStatus()
                        + ", 响应: " + response.getData());
            }
            ...
            if (token == null || expireTimeSeconds <= 0L) {
                throw new OperationFailedException("阿里云Token响应不完整: " + response.getData());
            }
            return new TokenCache(token, expireTimeSeconds * 1000L);
        } catch (ClientException e) {
            log.error("调用阿里云API失败: {}", e.getMessage(), e);
            throw new OperationFailedException("调用阿里云API失败: " + e.getMessage(), e);
        } catch (Exception e) {
            throw new OperationFailedException("获取阿里云Token失败: " + e.getMessage(), e);
        }
    }
}
