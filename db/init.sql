...
INSERT INTO `xiaozhi_jpa`.`sys_template` (`userId`, `templateName`, `templateDesc`, `templateContent`, `category`, `isDefault`) VALUES
...
-- 插入菜单权限
INSERT INTO `xiaozhi_jpa`.`sys_permission` (`parentId`, `name`, `permissionKey`, `permissionType`, `path`, `component`, `icon`, `sort`, `visible`, `status`) VALUES
-- 主菜单
...
-- 插入角色
INSERT INTO `xiaozhi_jpa`.`sys_auth_role` (`roleName`, `roleKey`, `description`, `status`) VALUES
('管理员', 'admin', '系统管理员，拥有所有权限', '1'),
('普通用户', 'user', '普通用户，拥有基本操作权限', '1');

-- 管理员角色权限（所有权限）
INSERT INTO `xiaozhi_jpa`.`sys_role_permission` (`roleId`, `permissionId`)
SELECT 1, permissionId FROM `xiaozhi_jpa`.`sys_permission`;

-- 将admin用户设为管理员角色
UPDATE `xiaozhi_jpa`.`sys_user` SET `roleId` = 1 WHERE `username` = 'admin';

-- 将其他用户设为普通用户角色
UPDATE `xiaozhi_jpa`.`sys_user` SET `roleId` = 2 WHERE `username` != 'admin';

SET FOREIGN_KEY_CHECKS = 1;
-- 删除 sys_device 表的 lastLogin 字段
-- 该字段已废弃，设备最后活跃时间改用 updateTime 字段替代
-- 执行时间：2026-02-25
ALTER TABLE `xiaozhi_jpa`.`sys_device` DROP COLUMN `lastLogin`;
