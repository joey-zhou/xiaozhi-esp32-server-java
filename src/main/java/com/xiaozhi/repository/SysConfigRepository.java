...
public interface SysConfigRepository extends JpaRepository<SysConfig, Integer>, JpaSpecificationExecutor<SysConfig> {
    ...
    default int add(SysConfig newLlmConfig) {
        save(newLlmConfig);
        return 1;
    }

    default int update(SysConfig sysConfig) {
        save(sysConfig);
        return 0;
    }
     ...
     * 重置默认配置
     *
     * @param resetConfig 包含 userId、configType、modelType 的配置对象
     */
    default void resetDefault(SysConfig resetConfig) {
        ...
        // 调用 JPA Query 方法重置默认配置（支持 modelType 条件）
